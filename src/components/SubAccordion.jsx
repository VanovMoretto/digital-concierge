...
function SubAccordion({ icon, title, children }) {
  ...
  // Usa as classes do SubAccordion
  const headerClasses = `${styles.subAccordionHeader} ${isOpen ? styles.open : ''}`;
  const contentClasses = `${styles.subAccordionContent} ${isOpen ? styles.open : ''}`;
